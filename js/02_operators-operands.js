 ...
 *
 * - &&(AND)はオペランドの途中でfalse(falsy)な値が見つかった場合、
 *   処理を中止しfalseを返す（短絡評価）
 * @example false && true // false（右のtrueは評価されない）
 *
 * - ||(OR)はオペランドの途中でtrue(truthy)な値が見つかった場合、処理を中止し
 *   trueを返す（短絡評価）
...
/**
 * オプショナルチェイニング演算子（?.）
 * - オブジェクトのプロパティや関数の呼び出しの際に、途中のオブジェクトが
 *   nullまたはundefinedの場合にエラーを防ぐための演算子。
 * - 例えば、obj?.propは、objがnullまたはundefinedでない場合にobj.propを返し、
